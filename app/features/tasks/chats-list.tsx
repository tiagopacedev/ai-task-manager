...
export function ChatsList() {
  ...
  useEffect(() => {
    if (
      ...
      const { chatId } = fetcher.formData ? Object.fromEntries(fetcher.formData) : {}
  ...
  function handleInputKeyDown(e: React.KeyboardEvent<HTMLInputElement>, chatId: string) {
    if (e.key === 'Enter') {
      e.preventDefault()
      submitTitle(chatId)
  ...
  function submitTitle(chatId: string) {
    if (inputValue.trim() !== '') {
      setPrevTitles((prev) => ({
        ...prev,
        [chatId]: optimisticTitles[chatId] ?? chats.find((c) => c.id === chatId)?.title ?? '',
      }))
